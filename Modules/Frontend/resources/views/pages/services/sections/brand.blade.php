...
@section('title', __('section.services_page_brand_section'))
...
@include('core::layout.partials.breadcrumb', [
    ...
    'breadcrumbs' => [
        ...
        ['label' => __('section.services_page_brand_section')]
    ]
])
@endsection
...
<div class="pb-12">
    <form action="{{ route('admin.services_page.update_brand_section', ['section_id' => $section->id, 'code' => $code]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <x-card :heading="__('section.services_page_brand_section')" :route="route('admin.services_page.index')" btnType="back">
            <div class="row gy-5">
                ...
                <div class="col-md-12 {{ hideDivLang($code) }}">
                    ...
                    <select class="form-control conca-select2" name="brands[]" id="brands" multiple>
                        @foreach ($brands as $brand)
                            <option value="{{ $brand->id }}" {{ in_array($brand->id, $selectedBrands) ? 'selected' : '' }}>{{ $brand->getTranslation($code)->title }}</option>
                        @endforeach
                    </select>
