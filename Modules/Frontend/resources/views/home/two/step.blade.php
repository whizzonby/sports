...
@section('title', __('section.home_two_step_section'))
...
@include('core::layout.partials.breadcrumb', [
    ...
    'breadcrumbs' => [
        ...
        ['label' => __('section.home_two_step_section')]
    ]
])
@endsection
...
<div class="pb-12">
    <form action="{{ route('admin.home_two.update_step_section', ['section_id' => $section->id, 'code' => $code]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <x-card :heading="__('section.home_two_step_section')" :route="route('admin.home_two.index')" btnType="back">
            <div class="row gy-5">
                ...
                <div class="col-md-4 {{ hideDivLang($code) }}">
                    <x-image-uploader name="thumb_shape_1" :label="__('attribute.thumb_shape_1')"  :value="$section?->default_content?->thumb_shape_1 ?? ''"/>
                    <x-input-error field="thumb_shape_1" />
                </div>
                <div class="col-md-4 {{ hideDivLang($code) }}">
                    <x-image-uploader name="thumb_shape_2" :label="__('attribute.thumb_shape_2')"  :value="$section?->default_content?->thumb_shape_2 ?? ''"/>
                    <x-input-error field="thumb_shape_2" />
                </div>
                <div class="col-md-4 {{ hideDivLang($code) }}">
                ...
                <div class="col-md-12">
                    ...
                    <x-input-msg :text="__('admin.title_shape_insert')" />
                ...
                @for ($i = 1; $i <= 4; $i++)
                    <div class="col-md-6">
                        <x-input-label
                            :for="'step_subtitle_' . $i"
                            :value="__('attribute.step_subtitle_' . $i)"
                        />
                        <x-text-input
                            :id="'step_subtitle_' . $i"
                            :name="'step_subtitle_' . $i"
                            :value="$section?->getTranslation($code)?->content?->{'step_subtitle_' . $i} ?? ''"
                        />
                        <x-input-error :field="'step_subtitle_' . $i" />
                    </div>
