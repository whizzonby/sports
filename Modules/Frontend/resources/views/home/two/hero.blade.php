...
@section('title', __('section.home_two_hero_section'))
...
@include('core::layout.partials.breadcrumb', [
    ...
    'breadcrumbs' => [
        ...
        ['label' => __('section.home_two_hero_section')]
    ]
])
@endsection
...
<div class="pb-12">
    <form action="{{ route('admin.home_two.update_hero_section', ['section_id' => $section->id, 'code' => $code]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <x-card :heading="__('section.home_two_hero_section')" :route="route('admin.home_two.index')" btnType="back">
            <div class="row gy-5">
                ...
                <div class="col-md-4 {{ hideDivLang($code) }}">
                    <x-image-uploader name="thumb_shape" :label="__('attribute.thumb_shape')"  :value="$section?->default_content?->thumb_shape ?? ''"/>
                    <x-input-error field="thumb_shape" />
                </div>
                <div class="col-md-4 {{ hideDivLang($code) }}">
                    <x-image-uploader name="title_shape_1" :label="__('attribute.title_shape_1')"  :value="$section?->default_content?->title_shape_1 ?? ''"/>
                    <x-input-error field="title_shape_1" />
                </div>
                <div class="col-md-4 {{ hideDivLang($code) }}">
                    <x-image-uploader name="title_shape_2" :label="__('attribute.title_shape_2')"  :value="$section?->default_content?->title_shape_2 ?? ''"/>
                    <x-input-error field="title_shape_2" />
                </div>
