<form action="{{ route('admin.payments.update', ['payment' => $payment->id]) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row gy-6">
        ...
        <div class="col-lg-6">
            <x-input-label for="stripe_client" :value="__('attribute.stripe_client')" />
            <x-text-input id="stripe_client" name="stripe_client" :value="$data->stripe_client" />
            <x-input-error field="stripe_client" />
        </div>
        <div class="col-lg-6">
            <x-input-label for="stripe_secret" :value="__('attribute.stripe_secret')" />
            <x-text-input id="stripe_secret" name="stripe_secret" :value="$data->stripe_secret" />
            <x-input-error field="stripe_secret" />
        </div>
        <div class="col-lg-6">
