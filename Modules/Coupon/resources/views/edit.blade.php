...
<form action="{{ route('admin.coupons.update', ['coupon' => $coupon->id]) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <x-card :heading="__('admin.edit_coupon')" :route="route('admin.coupons.index')" btnType="back">

        <div class="row gy-5 mb-5">
            <div class="col-md-12">
                <x-image-uploader name="image" :label="__('attribute.image')" :value="$coupon->image" />
            ...
            <div class="col-md-6">
                ...
                <x-text-input id="coupon_code" name="coupon_code" :value="old('coupon_code', $coupon->coupon_code)"/>
                <x-input-error field="coupon_code" />
            </div>
            <div class="col-md-6">
                <x-input-label for="min_price" :value="__('attribute.min_price')" />
                <x-text-input type="number" id="min_price" name="min_price" :value="old('min_price', $coupon->min_price)"/>
                <x-input-error field="min_price" />
            </div>
            <div class="col-md-6">
                <x-input-label for="discount" :value="__('attribute.discount')" />
                <x-text-input type="number" id="discount" name="discount" :value="old('discount', $coupon->discount)"/>
            ...
            <div class="col-md-6">
                ...
                <select id="discount_type" name="discount_type" class="form-select">
                    ...
                    <option value="amount" {{ old('discount_type', $coupon->discount_type) == 'amount' ? 'selected' : '' }}>{{ __('admin.amount') }}</option>
                </select>
                <x-input-error field="discount_type" />
            </div>
            <div class="col-md-4">
                <x-input-label for="per_user_limit" :value="__('attribute.per_user_limit')" />
                <x-text-input id="per_user_limit" name="per_user_limit" :value="old('per_user_limit', $coupon->per_user_limit)"/>
                <x-input-error field="per_user_limit" />
            </div>
            <div class="col-md-4">
                <x-input-label for="start_date" :value="__('attribute.start_date')" />
                <x-text-input id="start_date" name="start_date" :value="old('start_date', $coupon->start_date)"/>
                <x-input-error field="start_date" />
            </div>
            <div class="col-md-4">
                <x-input-label for="end_date" :value="__('attribute.end_date')" />
                <x-text-input id="end_date" name="end_date" :value="old('end_date', $coupon->end_date)"/>
        ...
        <div class="row">
            <div class="col-sm-4">
                <div class="mb-4">
                    <x-input-switch name="status" :label="__('attribute.status')" :checked="$coupon->status"/>
...
<script>
    flatpickr("#start_date", {
        enableTime: false,
        dateFormat: "Y-m-d",
        defaultDate: "{{ old('start_date', $coupon->start_date) }}"
    });
    flatpickr("#end_date", {
        enableTime: false,
        dateFormat: "Y-m-d",
        defaultDate: "{{ old('end_date', $coupon->end_date) }}"
    });
