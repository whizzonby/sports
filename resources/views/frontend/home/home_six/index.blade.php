...
@section('header')
   @include('frontend.layouts.headers.header-6', ['main_menu' => $main_menu])
@endsection
...
@section('content')
    ...
    @php
        $slugs = [
            'hero', 'text-slider', 'about', 'portfolio', 'services', 'banner',
            'fun-fact', 'brand', 'team'
        ];
    @endphp
    ...
    @foreach ($slugs as $slug)
        ...
        @if ($section?->status)
            @includeIf('frontend.home.home_six.sections.' . $slug, [
...
@section('footer')
    @include('frontend.layouts.footers.footer-6', [
