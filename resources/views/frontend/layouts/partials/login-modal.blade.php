  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="loginModalLabel">{{ __('frontend.login') }}</h1>
                ...
                <form method="POST" action="{{ route('user.login.post') }}">
                    ...
                    <div class="tp-login-suggetions d-sm-flex align-items-center justify-content-between mb-30">
                        <div class="tp-login-remeber">
                            ...
                            <label for="remember_me">{{ __('frontend.remember_me')}}</label>
                        </div>

                        @if (Route::has('password.request'))
                        <div class="tp-login-forgot">
                            <a href="{{ route('user.password.request') }}">{{ __('frontend.forgot_password') }}</a>
                        </div>
                        @endif
                    </div>

                    <div class="tp-login-bottom">
                        <button type="submit" class="tp-login-btn w-100">{{ __('frontend.sign_in') }}</button>
                    </div>
                </form>

                <div class="tp-login-top text-center mt-30">
                    <p>{{ __('frontend.dont_have_an_account') }}
                        <span>
                            <a href="{{ route('user.register') }}">{{ __('frontend.create_free_account') }}</a>
                        </span>
                    </p>
                </div>
            </div>
